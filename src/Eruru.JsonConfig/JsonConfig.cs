...
#pragma warning restore CA1724 // 类型名与命名空间名称整体或部分冲突
		...
		TimeSpan AutoReloadDebouncerTime = TimeSpan.FromSeconds (1);
		...
		public async Task<JsonConfig<TConfig, TContext>> BuildAsync (CancellationToken? cancellationToken = null) {
			...
			try {
				...
				await TrySaveAsync (cancellationToken).ConfigureAwait (false);
				JsonConfigSource.OnChanged += JsonConfigSource_OnChanged;
				return this;
			} finally {
		...
		public async Task<bool> TrySaveAsync (CancellationToken? cancellationToken = null) {
			CheckDisposed ();
			CheckBuild ();
			CancellationTokenSource? cancellationTokenSource = null;
			try {
				...
				try {
					CheckDisposed ();
					return await TrySaveAsync (Value).ConfigureAwait (false);
				} finally {
					SemaphoreSlim.Release ();
				}
			} finally {
		...
		async Task<bool> TrySaveAsync (TConfig? value) {
		...
		void JsonConfigSource_OnChanged (object? sender, EventArgs e) {
			...
			_ = AutoReloadDebouncerAsync ().ContinueWith (
				static _ => { }, CancellationToken.None, TaskContinuationOptions.RunContinuationsAsynchronously, TaskScheduler.Default
			);
		}
